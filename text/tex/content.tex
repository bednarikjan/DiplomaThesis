...
In order to test the real world performance of the OLS two datasets were obtained using a basic two-cameras setup. 

The rest of the tests were performed in the semi-simulated environment, where the P\&T

\paragraph{Real world environment}

The first dataset (\textsc{meadow}) was obtained on the meadow located between districts Bystrc and Žebetín (Brno, Czech Republic). The CUs were precisely positioned using differential GPS sensor so that the base would be $30~m$ long. To create the second dataset (\textsc{pitch}) the athletics stadium VUT SAPPV located in district Královo Pole (Brno, Czech Republic) was chosen since the DGPS sensor was not available and thus the marks painted on the running pitch were used to station the CUS which were placed $10~m$ apart (see Figure~\ref{fig:dataset_zebetin_vut}).
...
The system was tested against both static and dynamic targets, and in both cases only horizontal position was considered. 
